...
namespace MyMQTTClientNr1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            // Fill combobox with available DAQ channels
            ...
            MqttClient sMqttClient = new MqttClient(sBroker);

            // Eventhandler toevoeggen bij message received
            sMqttClient.MqttMsgPublishReceived += clientReceivedMessage;

            // maak een unieke id aan
            ...
            {
                string sHeader = "Current time;AUX Temp;FPA Temp;Last FFC time;Calculated Temp;Min Temp;Max Temp;Ref Temp;" + Environment.NewLine;
                File.WriteAllText(msLogFileName, sHeader);
            }
        }


        private void refreshPicBox(uint[] mai16BitData)
        {
            // eerste 2 woorden van een lijn zijn rommel.
            int value;
            int maxValue = 0, minValue = 65535;
            int maxValueIdx = 0, minValueIdx = 0;

            for (int i = 0; i < mai16BitData.Length; i++)
            {
                if (IGNORE_TWO_FIRST_BYTES_IN_LINE)
                {
            ...
            for (int i = 0; i < mai16BitData.Length; i++)
            {
                ...
                value = (int)(((float)mai16BitData[i] - (float)minValue) * scale);
                r = 0; g = 0; b = 255;
                if (value <= 255)
                {
        ...
        private void clientReceivedMessage(object sender, MqttMsgPublishEventArgs e)
        {
            if (e.Topic == MQTT_MSG_PREFIX + "/FrameBuffer")
            {
                ...
                // Make memory stream from received bytes
                MemoryStream sMsFb = new MemoryStream(abMsg);

                // Save values as UINT16
                uint[] mai16BitData = new uint[abMsg.Length / 2];
                using (BinaryReader br = new BinaryReader(sMsFb))
                {
                    for (int i = 0; i < abMsg.Length / 2; i++)
                        mai16BitData[i] = br.ReadUInt16();
                }
                sMsFb.Dispose();
